...
**v1.1.0 is being prepared** (`docs/RELEASE_NOTES_v1.1.0.md`): adds the standard-MTU `wireguard-uci-*-mtu1440.prg` variants and a `wireguard-mtu1440.d64` — **these require the unmerged GideonZ/1541ultimate#807 firmware** (see the warning below); the default variants run on released firmware. **[v1.0.0](https://github.com/JC-000/c64-wireguard/releases/tag/v1.0.0) was the first tagged release** (2026-07-28): ready-to-run `.prg` and `.d64` artifacts for both network backends in REU and stock-C64 (no-REU) variants. The released UCI/REU build repeated the full handshake + bidirectional transport on hardware post-tag (`docs/RELEASE_NOTES_v1.0.0.md` §Verification).
