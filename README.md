...
**Chunked send — `UCI_CHUNKED_WRITE=1` (issue [#70](https://github.com/JC-000/c64-wireguard/issues/70), firmware [GideonZ/1541ultimate#807](https://github.com/GideonZ/1541ultimate/issues/807)).** The #807 spike firmware adds `NET_CMD_WRITE_SOCKET_CHUNK` (`$16`): the C64 pushes a datagram in parts of at most 888 bytes, each announcing the total, and the firmware emits it as **one** wire datagram of up to 1472 bytes. `make BACKEND=uci UCI_CHUNKED_WRITE=1` builds an adapter that sends *every* datagram that way: `NET_UDP_SEND_MAX = 1472`, `WG_MTU = 1440`, and peers must then use `MTU = 1440`. It is off by default and the default build is byte-identical with or without the flag — the flag is only for a device running a firmware with `$16` (stock 3.15 does **not** have it and answers `21,UNKNOWN COMMAND`; a chunked build there fails its first send, the 148-byte handshake, with `SEND FAILED, NET ERR $8E` on screen). The RAM for the larger buffers came from typing the outbound message straight into `ip_packet_buf` instead of a private text buffer, so the default build gained ~830 bytes of BSS slack as well. Host tools read the *built* values from `build/labels.txt` (`tools/c64_caps.py`; `python3 tools/c64_caps.py [labels.txt]`), never from the `.inc` files, because the same sources now describe two MTUs.
...
- **1472 bytes** is the largest datagram that reaches the device at all: lwIP is built with `IP_REASSEMBLY = 0`, so anything that fragments is dropped before a socket sees it. That applies to every network service on the device, not just UCI, and is why 1440 rather than 1420-plus is the ceiling even with chunked sends.
