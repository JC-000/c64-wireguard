...
The shipped build links the sibling crypto libraries [c64-x25519](https://github.com/JC-000/c64-x25519) (v0.16.0) and [c64-ChaCha20-Poly1305](https://github.com/JC-000/c64-ChaCha20-Poly1305) (v0.11.0) as archives per the [c64-lib-contract](https://github.com/JC-000/c64-lib-contract) conventions — every reachable multiply on the X25519 and Poly1305 paths is the contract's constant-time `ct_mul_8x8` body. The in-tree crypto remains available behind `USE_*_SIBLING=0` as a legacy/dev configuration.
...
**Standard WireGuard MTU achieved on both backends (2026-09-03).** With `UCI_CHUNKED_WRITE=1` the UCI tunnel runs at `WG_MTU` 1440 (1472-byte datagrams) bidirectionally, with the peer left at its 1420 default — no per-peer MTU configuration needed; verified 60/60 twice at 48 MHz on the U64E (PR [#112](https://github.com/JC-000/c64-wireguard/pull/112)). The ip65 backend gets the same MTU **natively** via `WG_MTU1440=1`, with no firmware involved at all, since its caps are 1472/1472 (PR [#119](https://github.com/JC-000/c64-wireguard/pull/119)). **Since v1.2.0 that is the ip65 DEFAULT**: `WG_MTU1440` defaults to `1` under `BACKEND=ip65`, so a bare `make` is the MTU-1440 RR-Net build and the build default no longer disagrees with the shipped artifact. `make WG_MTU1440=0` still builds the 860 RR-Net PRG — byte-identical to what a bare `make` produced before the flip — but no released artifact corresponds to it. **Shipped builds are mixed, and the reason is UCI-only.** As of v1.2.0 four of the six released PRGs are MTU 1440 — both RR-Net builds (natively, no firmware involved) and both UCI `*-mtu1440` builds. The two that stay at 860 are the plain UCI builds, because no released firmware carries the chunked write. **None of that reasoning applies to RR-Net**, which talks to a CS8900a: do not set a peer to `MTU = 860` for an RR-Net build, and do not go looking for a firmware requirement it does not have. Every UCI build, at either MTU, still needs firmware 3.15+, itself not a public release (see the box below). See [Chunked send](#chunked-send-ucichunkedwrite1) and [Tunnel MTU](#tunnel-mtu).
...
make                 # ip65/RR-Net backend, REU profile, sibling crypto (default)
                     # MTU 1440 since v1.2.0 — this IS the shipped RR-Net shape, and
                     # `make REU=0` reproduces the hardware-validated artifact
                     # wireguard-rrnet-noreu-mtu1440.prg byte-for-byte.
make BACKEND=uci     # Ultimate 64 / C64U UCI backend instead of ip65 — MTU 860
...
make WG_MTU1440=0                      # ip65 at MTU 860 — the opt-OUT; NOT a shipped artifact
...
| `WG_MTU1440` | **default is backend-dependent: `1` under `BACKEND=ip65`, `0` under `BACKEND=uci`** | Lifts `WG_DATAGRAM_CAP` from 892 to 1472 and the tunnel `WG_MTU` from 860 to 1440. Under `BACKEND=ip65` the flag alone suffices (ip65's caps are natively 1472/1472, so **no firmware dependency of any kind** — the `#807` requirement below is UCI-only), and it is **on by default** because 860 was never an ip65 number: it derives from the UCI `SOCKET_WRITE` payload cap, which ip65 does not have. **The 1440 ip65 build is the one validated on physical RR-Net hardware on 2026-09-05** (three runs, handshake to ACTIVE and transport both ways), the only ip65 configuration with hardware evidence, and the only RR-Net MTU v1.2.0 ships. `make WG_MTU1440=0` still builds the 860 ip65 PRG — a command-line assignment beats the default — but no released artifact corresponds to it, and it has been exercised under bridged-Ethernet VICE only. Under `BACKEND=uci` the default is `0` and setting it to `1` must be paired with `UCI_CHUNKED_WRITE=1` — alone it is a make error, because plain `SOCKET_WRITE` caps sends at 892. Spelling out a backend's default value is byte-identical to not spelling it out (`tools/test_build_mtu1440.py` pins both directions). RAM: the ip65 1472 build's `APP_BSS` ends at `$9E8B` with 372 B of `MAIN_AREA_HI` free (fits since `msg_input_buf` went in [#112](https://github.com/JC-000/c64-wireguard/pull/112)). See [Tunnel MTU](#tunnel-mtu). |
...
| `src/net/uci/net.s` | UCI backend: net ABI over the Ultimate Command Interface sockets. Hardware-proven on the U64E — but so is ip65 since 2026-09-05, so this is no longer the *only* one; pick a backend on what hardware you have, not on this row |
...
# uci_short_read) run serially afterwards, then the default build
# is restored.
#
...
#
# NOTE the gate leaves build/ as the ip65 DEFAULT build, which since v1.2.0
# is BACKEND=ip65 at MTU 1440 (sha256 f3f76459..., the same bytes as the
# released wireguard-rrnet-reu-mtu1440.prg) — it used to be the MTU-860 one.
# The runner prints the sha it leaves behind. Rebuild explicitly before any
# hardware run that needs another backend, another REU profile or MTU 860.
...
At 1 MHz. The handshake and turbo figures below were measured on hardware on 2026-08-15 at the **v0.11.2 / v0.9.0** pins, and the per-primitive numbers are hardware-anchored to the c64-x25519 v0.8.0 release and the v1.0.0 runs. Those numbers used to stay current by a byte-identity argument — x25519's PRG was unchanged from v0.8.0 through v0.11.2 — and **that argument no longer holds at the v0.16.0 pin**: x25519 v0.15.0 changed the PRG. The figures below have **not** been re-measured on hardware at the new pins and should be read as the last measured values, not as current ones.
...
The 51.7x from a 48x clock is not a measurement error, but it is **not fully explained either, and this text used to imply that it was**. Badline DMA costs a *fixed* amount at the VIC's own 1 MHz rate, so it takes ~6% of the CPU at 1 MHz and ~0.13% at 48 MHz: predicted `48 x 1.063 = 51.0x` against 51.7x measured. That 1.063 is the same ~6% figure the VIC blanking section derives independently, and the 1541ultimate lane reproduces it exactly from 25 badlines x 40 cycles out of 17,030 cycles per NTSC frame.

**A 1.4% residual remains, and the measured speedup EXCEEDS the prediction — the wrong sign for anything that would slow the top of the range.** Earlier wording presented 51.0 against 51.7 as confirmation; it is a residual we read as agreement because it was close and corroborated a figure we liked. Two open threads bear on it. The 1541ultimate lane measures the clock at indices labelled 40 and 48 as delivering exactly 39.00x and 47.00x PHI2 (four instruction mixes, three at identical cycle counts, agreeing to 0.03%, so neither a memory stall nor a cycle-accounting error); at 47x the residual widens to 3.5% rather than closing. And our 51.7x is an end-to-end X25519 figure whose 1 MHz arm is the one badlines actually bite, so anything making that arm slower than the model assumes inflates the ratio. The clean discriminator, not yet run, is our no-REU benchmark at index 13 — labelled 32, measured exactly 32.00x — where the clock is not in dispute and the 1.063 factor should predict the point exactly if it is right.
...
**ip65 / RR-Net** (`BACKEND=ip65`, the default — VICE-testable, and **hardware-validated on a physical CS8900a since 2026-09-05**; for a bench run see [`docs/hardware-validation-runbook.md`](docs/hardware-validation-runbook.md) §5c, which covers the preconditions the tooling cannot check for itself): UDP via [ip65](https://github.com/cc65/ip65), driving the RR-Net CS8900a ethernet adapter. The ip65 library is built as a standalone binary blob (ca65/ld65) and linked into the final PRG at $2000 via ca65's `.incbin` directive in `src/net/ip65/ip65_blob.s`. A 10-entry jump table provides: init, process, DHCP, DNS, UDP add/remove listener, UDP send, and helper wrappers. The UDP receive callback fires during `ip65_process` while ip65 owns the zero page; it copies incoming data to `udp_recv_buf` and sets a flag for the main loop — no crypto ZP is touched.
...
> **⚠️ REU build known-failing on fw 3.15 at turbo.** On firmware 3.15 the REU-DMA multiply (`REU=1`, the default) produces wrong products at 48 MHz turbo, so the handshake fails (the responder rejects the C64's Type-1 with `InvalidTag`) — [#69](https://github.com/JC-000/c64-wireguard/issues/69). The cause is a DMA post-execute settle the shared `reu_mul` primitive doesn't yet perform (tracked in c64-x25519; c64-lib-contract §8.2). The bump to x25519 **v0.16.0 lands that settle** (their v0.12.0: read `reu_status` `$DF00`, confirm bit 6, spin bounded, post-execute settle), so #69 is now **testable** at `REU=1` rather than blocked. It is **not** verified fixed — no hardware run has been made at the new pin — and the settle exists only in the REU profiles, so it changes nothing about the `REU=0` build we actually ship. **Continue to build with `REU=0` for hardware** (it also scales better at turbo) until a hardware run says otherwise. The gate's counts and the hardware rows below cover the `REU=0` path and the non-REU logic; they do **not** certify the REU build on 3.15.

### Tunnel MTU

**This section is about the UCI backend. On RR-Net (`BACKEND=ip65`) the default AND shipped build is MTU 1440 and needs no Ultimate firmware at all** — ip65's 1472-byte caps are native, so none of the `SOCKET_WRITE` arithmetic below applies to it, and the `#807` dependency is UCI-only. That configuration is the one validated on physical RR-Net hardware on 2026-09-05.
...
The tunnel MTU is the smaller of the two minus the overhead: `WG_MTU = min(892, 1472) − 32 = 860`, derived in `src/constants.inc` from `net_caps.inc` — it is **send-bound**. Both ceilings above are UCI firmware properties; ip65 declares 1472/1472 and so lands at 1440. The host-side tools read the same two files (`tools/c64_caps.py`) so no Python constant can drift from the assembly.
...
```bash
make BACKEND=uci REU=0 UCI_CHUNKED_WRITE=1                                # build/, msg_port 9999 (wire port 9999 since #113)
